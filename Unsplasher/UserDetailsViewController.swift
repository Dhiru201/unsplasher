...
class UserDetailsViewController: UIViewController , UITableViewDelegate, UITableViewDataSource{
	...
	override func viewDidLoad() {
		...
		self.userImageView.estimatedSectionHeaderHeight = 35
		self.updateUserView()
	...
	func deviceSetup(){
		...
		self.setHeaderSize()
		self.setHeaderUserImageSize()
		self.userImage.toCircle()
	}
	
	private func setHeaderUserImageSize(){
		var size: CGFloat!
		...
		self.userImage.frame = CGRect(x: self.userImage.frame.origin.x,
		                              y: self.userImage.frame.origin.y,
		                              width: size,
		                              height: size)
	}
	...
	private func setHeaderSize(){
		...
		self.userInfoView.frame = CGRect(x: 0, y: 0, width: self.userInfoView.frame.width, height: headerHeight)
	}
	
	private func headerLabelAlingment() {
		for label in [userName, location, bio]{
			if UIDevice.current.userInterfaceIdiom == .pad {
	...
	func setSegmentedControl(){
		self.segmentedControl = UISegmentedControl(frame: CGRect(x: 0, y:0, width: (self.view.frame.width), height: 35))
